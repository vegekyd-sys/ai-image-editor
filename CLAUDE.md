...
**核心架构**：`AnimationState` 接口提升到 Editor 层管理，AnimateSheet 和 CUI 共享状态。状态流：`idle` → `generating_prompt`（Agent 后台写脚本）→ `ready`（可编辑/提交）→ `submitting` → `polling`（PiAPI 渲染 ~5 分钟）→ `done`（视频完成）。

**视频 Timeline 条目**：视频生成后作为 timeline 最后一个条目（`__VIDEO__` sentinel），可滑动到达。Canvas 渲染 `<video>` 元素，用最后一个 snapshot 作为 poster（避免黑屏首帧），正中间播放按钮点击播放。

**AnimateSheet 交互**：底部 sheet 卡片（28dvh，无蒙版不遮 canvas），X 悬浮固定右上角（`position: absolute`），所有内容可滚动。内容顺序：filmstrip 缩略图（@1-@N）→ 故事脚本 textarea → 时长/费用 → 生成按钮。点 X 关闭并回到最后 snapshot。`showAnimateSheet` 或 `isViewingVideo` 时自动显示。
...
**视频 API Provider**：默认 PiAPI（Kling 3.0 Omni，`version: '3.0'`，`enable_audio: true`，`@image_N` 引用）。设 `ANIMATE_PROVIDER=kling` 可切到 Kling 官方 API（O1，`kling-video-o1`，`<<<image_N>>>` 引用）。两套代码共存（`piapi.ts` + `kling.ts`），路由层通过 env var 切换。
...
**关键限制**：images 数组限 4 张，必须用 Supabase Storage URL。Agent 写脚本约 2 分钟（Bedrock Sonnet 多图 TTFT 慢）。
