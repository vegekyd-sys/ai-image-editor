...
**AnimateSheet 交互**：底部 sheet 卡片（无蒙版不遮 canvas），点 X 关闭并回到最后 snapshot。`showAnimateSheet` 或 `isViewingVideo` 时自动显示。「重新生成视频」保持 sheet 打开（done→idle 时 setShowAnimateSheet(true)），并从最新 snapshots 刷新 imageUrls。时长选项：3s/5s/7s/10s/15s/智能。
...
**视频 API Provider**：默认 Kling 直连（`kling-v3-omni`，`sound: 'on'`，`<<<image_N>>>` 引用，$0.112/s）。设 `ANIMATE_PROVIDER=piapi` 可切回 PiAPI（`@image_N` 引用，$0.168/s，路由层自动转换格式）。两套代码共存（`piapi.ts` + `kling.ts`）。
