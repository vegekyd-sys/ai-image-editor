...
**AnimateSheet 交互**：底部 sheet 卡片（33dvh，无蒙版不遮 canvas），内容：故事脚本 textarea → filmstrip 缩略图（@1-@N）→ 时长/费用 → 生成按钮。点 X 关闭并回到最后 snapshot。`showAnimateSheet` 或 `isViewingVideo` 时自动显示。
...
**视频保存**：Save 按钮通过 `/api/proxy-video` 服务端代理下载（绕过 CORS），iOS 弹出分享表可保存到相册。StatusBar 显示"准备视频下载中..."。

**CUI 集成**："在 Chat 里看 ↗" 只切视图不触发 Agent。视频完成时自动添加 CUI 消息（含 .mp4 URL，AgentChatView 自动渲染 inline video player）。

**关键限制**：images 数组限 4 张，必须用 Supabase Storage URL，10s 视频约 $1.68/次（`PIAPI_API_KEY`）。Agent 写脚本约 2 分钟（Bedrock Sonnet 多图 TTFT 慢）。
