...
**IndexedDB 本地缓存（2026-02-23）**：解决 Supabase 上传未完成时退出、重进图片消失的问题。`src/lib/imageCache.ts` 实现双层缓存：内存 Map（同步写入，同 session 导航零延迟命中）+ IndexedDB（异步持久化，跨 session）。5 处写入点：原图上传、agent 生图、commit tip draft、tip preview 生成完成、pendingImage。读取在 `loadProject().then()` 里 patch 缺失图片，Editor 第一次渲染就拿到完整数据。项目列表页同样 patch 缺失的 snapshot 缩略图。关键 bug：DB 名 `makaron-images` 已存在（store 叫 `"snapshots"`），版本号 1 不触发 `onupgradeneeded`，`"images"` store 从未创建，写入全部静默失败——修复为版本号 2。
