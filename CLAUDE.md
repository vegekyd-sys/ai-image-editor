...
**v1.0 Snapshot 动画（2026-02-23，已验证）**：当项目有 ≥3 个 snapshot 时，timeline dots 末尾出现 ▶ 按钮，点击弹出底部 AnimateSheet。流程：Gemini 分析最多 4 张 snapshot（均匀采样）→ 流式生成电影感故事 prompt（可编辑）→ 提交 PiAPI Kling 3.0 Omni（多图 @image_n 引用）→ 轮询约 5 分钟 → Sheet 内显示视频播放器。关键技术：`src/lib/piapi.ts`（PiAPI 客户端）、`/api/animate`（创建任务）、`/api/animate/[taskId]`（轮询）、`/api/animate/prompt`（Gemini 生成故事）、`src/components/AnimateSheet.tsx`、`project_animations` 表（Supabase）。关键限制：images 数组限 4 张（超过导致 PiAPI 报 input too large），必须用 Supabase Storage URL（不能 base64），10s 视频约 $1.68/次（`PIAPI_API_KEY` env var）。已实测生成效果：像素游戏元素从人物飞出，完全按故事 prompt 描述生成。
