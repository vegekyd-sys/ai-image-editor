...
name: ai-video-gen
description: >
  Makaron adapter with native support for OpenMontage's ai-video-gen
...
- Choose the Makaron model from the user request or active app selection, keep duration/aspect/reference count within that model contract, and submit one generation without inventing an intermediate image.
