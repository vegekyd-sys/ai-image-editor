...
name: dashscope
description: >
  Makaron adapter with adapted support for OpenMontage's dashscope
  craft skill, using existing Makaron tools and durable project outputs.
allowed-tools: read_file analyze_image generate_image list_voiceover_voices generate_voiceover transcribe_audio run_code write_file
metadata:
  makaron:
    icon: "▧"
    color: "#d946ef"
    ...
    sourceSkill: "dashscope"
    sourceKind: "agent-skill"
    supportLevel: "adapted"
    adapterFamily: "image"
    canonicalSkill: "explainer-video"
    tags: [openmontage, agent-skill, image, adapted]
---
...
- Map DashScope image intent to the Qwen image route, TTS intent to voiceover generation, and timestamped ASR intent to transcription. State that Makaron owns the provider route and do not claim direct DashScope endpoint execution.
