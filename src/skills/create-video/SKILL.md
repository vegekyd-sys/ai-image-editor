...
name: create-video
description: >
  Makaron adapter with native support for OpenMontage's create-video
...
- Treat prompt-only generation as first-class. Do not require a source image and do not ask for a second confirmation when the current request already authorizes generation.
