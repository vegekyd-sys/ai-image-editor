...
description: Analyze a reference video, extract transferable structure and pacing, propose original directions, and produce a new editable video without copying its surface design. Use the video-edit replication profile instead when the user wants measurable shot count, order, timing, framing, or motion matching.
...
the `replication` profile in `skills/video-edit/SKILL.md`.
