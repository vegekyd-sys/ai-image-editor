...
const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    en: { title: 'Skill Marketplace & New Home Page', items: [
      ...
      'Video covers: some skills show looping video previews right in the grid',
      'Skill menu: attach a skill to any prompt — upload your own .zip skill packs or pick from the marketplace',
      'Cross-navigation: jump between Home (explore) and Projects (your work) from either page',
      'Language setting moved into the user menu dropdown (top-right) for a cleaner top bar',
      'Performance: images load 90% faster via Supabase thumbnails, overlay renders only nearby slides, fonts self-hosted (no Google Fonts latency)',
      'Skeleton loading: shimmer grid placeholders while skills load — no more blank page flash',
    ]},
    zh: { title: 'Skill 市场 & 全新首页', items: [
      ...
      '视频封面：部分 Skill 在网格中直接播放循环视频预览',
      'Skill 菜单：给任意 Prompt 挂载 Skill — 支持上传自定义 .zip Skill 包或从市场选择',
      '页面互通：Home（探索）和 Projects（我的项目）之间可自由跳转',
      '语言设置移入右上角用户下拉菜单，顶栏更简洁',
      '性能优化：图片加载快 90%（Supabase 缩略图）、详情页只渲染附近 10 张、字体本地化（无 Google Fonts 延迟）',
      '骨架屏加载：Skill 加载时显示闪光占位卡片，不再闪白屏',
    ]},
  },
  {
    date: '2026-04-26',
