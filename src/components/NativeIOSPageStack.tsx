...
function isPrimaryNativeTabPath(path: string): boolean {
  const pathname = path.split('?')[0];
  return pathname === '/' || pathname === '/home' || pathname === '/projects';
}
...
export default function NativeIOSPageStack({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    setEntries((current) => {
      if (isPrimaryNativeTabPath(pathKey)) {
        const currentTop = current[current.length - 1];
        if (current.length === 1 && currentTop?.path === pathKey) {
          return [{ ...currentTop, node: children, pending: false, frozen: false, phase: 'base', x: 0 }];
        }
        return [makeEntry(pathKey, children, 'base')];
      }
