...
/**
 * Capture a frame from a video URL using <video> + canvas.
 * Routes external URLs through /api/proxy-video to avoid CORS.
 * Tries multiple seek positions to skip black fade-in frames.
 * Returns base64 JPEG or null on failure.
 */
export async function captureVideoFrame(videoUrl: string): Promise<string | null> {
  try {
    const isSameOrigin = videoUrl.includes('cdn.makaron.app') || videoUrl.includes('supabase.co') || videoUrl.startsWith('/');
    const src = isSameOrigin ? videoUrl : `/api/proxy-video?url=${encodeURIComponent(videoUrl)}`;
    ...
    const candidates = [0.5, 1.0, 1.5, dur * 0.1, dur * 0.2];
    for (const t of candidates) {
      if (t >= dur) continue;
      video.currentTime = t;
      await new Promise<void>(r => { video.onseeked = () => r(); setTimeout(r, 3000); });
      ctx.drawImage(video, 0, 0);
      const sample = ctx.getImageData(Math.max(0, canvas.width / 2 - 32), Math.max(0, canvas.height / 2 - 32), 64, 64).data;
      let sum = 0;
      for (let i = 0; i < sample.length; i += 4) sum += sample[i] + sample[i + 1] + sample[i + 2];
      if (sum / (sample.length / 4 * 3) >= 10) {
        const poster = canvas.toDataURL('image/jpeg', 0.75);
