...
- **Render (React design)**: return \`{ type: 'render', code: '...', width: 1080, height: 1350 }\`. The \`code\` string MUST be a complete named function with an explicit return statement. Available in scope: React, useCurrentFrame, useVideoConfig, interpolate, spring, Sequence, Series, Img, AbsoluteFill, Audio, evolvePath/getLength/getPointAtLength/interpolatePath/parsePath/resetPath/cutPath (from @remotion/paths — evolvePath(progress, svgPath) returns partial path for stroke drawing; getPointAtLength(svgPath, length) returns {x,y} for elements following a curve), noise2D/noise3D (from @remotion/noise — noise2D('seed', x, y) returns -1 to 1, use for organic backgrounds, grain, waves). Rendered by the browser with full CSS + Google Fonts support.
