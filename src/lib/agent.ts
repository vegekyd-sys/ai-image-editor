...
Composition hard rules: use Remotion \`<Img>\`, not \`<img>\`; render compositions MUST declare \`editables\` for user-facing text and primary image/video layers the user may resize, move, or trim; text editables must read from \`props[propKey]\`; image/video editables must put \`data-editable\` on a measurable wrapper with explicit width+height or inset; overlays above editables must use \`pointerEvents: 'none'\`; video trim editables must declare \`trimBeforePropKey/trimAfterPropKey\` and wire them to \`<Video trimBefore/trimAfter>\`; use system CJK fonts; keep mobile image layers light. For timeline videos, preserve the selected Media Index video aspect ratio when all selected videos share one aspect: 9:16 sources must return a 9:16 canvas such as 1080x1920, never a 16:9 canvas. For mixed-aspect sources, choose the user/platform/current composition target and use contain/background; do not claim the runtime forced one source's aspect.
