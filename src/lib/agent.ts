...
- Default model follows app selection, usually SeeDance 2.0 Fast (\`seedance-fast\`) at 720p; do not silently change it. Use \`seedance-2.5\` when the user asks for Seedance 2.5, a single 16-30 second generation, more than the older reference limits, or its dedicated edit/extend features. Use \`minimax-h3\` only when the selector or user explicitly asks for MiniMax/H3/Hailuo H3; it supports native 2K multimodal generation and a gated 768p preview tier.
...
- The script must have been shown to the user and confirmed before this tool is called, unless the user's current request explicitly asks for direct submission without confirmation or the system prompt supplies the trusted Skill template launch exception.`,
      inputSchema: z.object({
        ...
        video_resolution: z.enum(['480p', '720p', '768p', '1080p', '2k', '4k', 'auto']).optional().describe('Output resolution. Seedance 2.5 supports 480p/720p; MiniMax H3 defaults to 2k and has a gated 768p preview tier.'),
