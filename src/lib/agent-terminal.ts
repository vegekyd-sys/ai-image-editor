...
export interface ModelTerminationAssessment {
  ok: boolean;
  retryable: boolean;
  code?: 'stream_error' | 'missing_finish' | 'truncated' | 'provider_error' | 'content_filter' | 'unfinished_tool_turn' | 'attempt_budget_handoff' | 'non_retryable_tool_failure';
  detail?: string;
}
...
/**
 * A transport EOF is not proof that an agent finished. A real provider finish
 * event with `stop` is authoritative, even when the final step has no new
 * visible text. Workflow state and artifact heuristics never override it.
 */
