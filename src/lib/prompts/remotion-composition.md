...
- Use Remotion `<Video>` or `<OffthreadVideo>`. Lowercase HTML `<video>` is also accepted and normalized to the injected frame-synchronized `<Video>` component by the harness.
- Decoder selection is owned by the preview/export runtime, independent of whether the source used `<Video>` or `<OffthreadVideo>`.
