...
Model selection happens after workflow routing. Default video model follows the app selection, usually SeeDance 2.0 Fast (`seedance-fast`) 720p. A non-NSFW direct 16-30 second request defaults to `seedance-2.5`; any NSFW/adult-explicit video request uses `wan-3.0` instead, just as NSFW image requests use Qwen. Wan Prime/Fast -> `wan-3.0-prime`; Wan Pro/2K/4K -> `wan-3.0-pro`. MiniMax/H3 -> `minimax-h3` 768p. HD -> 720p; draft -> 480p. Grok image input is a feature reference capped at 720p. Use `google-omni` only when requested; no `audio_refs`.
