...
例：蛋糕特写 → 主标题 "太甜了！"（大字弹入）+ 副标题 "Nintendo World 大阪"（略小淡入）+ 角标 "Day 1"（角落滑入）
不同层级用不同位置、不同入场时机——有先有后，有主有次。
**所有文字都要大**——副标题/角标只比主标题略小（fontSize ≥ 48），不要出现明显小的文字。一眼看不清 = 太小。

**Q4 补充：花字和画面的关系**
花字必须跟画面内容有关系——是对画面的回应，不是随意贴上去的装饰。
放置时避免挡住人物主体，但如果有故事性（花字内容跟人物动作/表情呼应），碰到一点人没关系。
花字和画面无关 = 不通过。花字无故事性地挡住人脸 = 不通过。

**Q4 补充：结尾处理**
最后一个场景要有收尾感——慢推/慢缩/色彩渐淡/文字缓慢消失。
**禁止黑屏结尾**（纯黑 fade out 不适合短视频，用户会以为加载失败）。
正确做法：保持最后一张图在画面上，叠加结束感的动效（如慢慢模糊、色温变暖、文字最后浮现再淡出）。画面始终有内容。
...
After planning, review your scenes: **do any need visual elements beyond the user's existing photos?**
...
2. **花字有没有挡住人？** — 花字尽量不挡人物主体。但如果有故事性（文字跟人物呼应），碰到一点没关系。花字不要贴在顶部/底部边缘（安全区外）。花字和画面无关 = 不通过。
3. **有没有太小的文字？** — 所有文字都应该大到一眼看清。副标题不应该比主标题小太多。
4. **结尾是不是黑屏？** — 最后一帧必须有画面内容，不能 fade to black。
