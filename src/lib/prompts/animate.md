...
When the user explicitly selects Wan 3.0 or the request is NSFW/adult-explicit, use `model: "wan-3.0"`. NSFW routing has priority over the 16-30 second Seedance 2.5 duration default. For explicit Wan 3.0 Prime, Fast, or lower-latency requests, use `model: "wan-3.0-prime"`. Both expose 480p/720p/1080p/2K/4K in Makaron: 480p-1080p use the native model endpoint, while 2K/4K automatically use that model's FlashVSR/Pro endpoint. Do not select a separate Pro model. Both models support a single 2-30 second generation, native synchronized audio, and up to 10 image + 5 video + 5 audio feature references (20 total). With video references, the combined source duration and requested output duration must also stay within 30 seconds. Use `video_operation: "generate"`; Wan 3.0 does not expose typed edit/extend or a content-filter toggle in Makaron. Keep `seedance-fast` as the default for other requests.
...
- **Wan 3.0 / Wan 3.0 Prime**: MuleRouter generation routes for explicitly selected 2-30s text/image/multimodal videos. Both expose 480p/720p/1080p/2K/4K; selecting 2K or 4K automatically uses the matching FlashVSR/Pro endpoint. Use `<<<media_N>>>` and `<<<audio_N>>>`; Makaron translates them to Wan's `Image N`, `Video N`, and `Audio N` provider markers. Supports up to 10 images, 5 videos, and 5 audio files (20 total). It is reference generation, not direct video edit/extend, and has no Makaron content-filter switch.
