...
Native-audio contract: this script is the complete audio direction for `generate_animation`. Keep dialogue, narration, voice performance, music, ambience, and sound effects inside the script so the video model generates them with the picture. Never prepare this workflow with `list_voiceover_voices`, `generate_voiceover`, `generate_audio`, or `generate_music`, and never call those tools after submitting the video. A request for voice or music inside the final video is not a request for a separate audio asset.
