...
Execution behavior: in an ordinary CUI/editor request, write the complete visible script and wait for confirmation before calling `generate_animation`. Submit in the same turn only when the current request explicitly authorizes direct submission or the system prompt explicitly supplies a `Trusted Skill template launch`. The trusted template exception still pauses for missing required inputs, a long-video/multi-segment approval checkpoint, or an explicit request to review the script first.
