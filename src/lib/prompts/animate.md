...
Default model behavior: follow the app's selected video model, usually SeeDance 2.0 Fast (`seedance-fast`) at 720p. Treat `seedance-fast` and standard `seedance` as separate models, not resolutions. Generic "HD"/"高清"/"high quality" requests still use `seedance-fast` 720p. Use standard `seedance` only when the user explicitly asks for 1080p, standard/full SeeDance, or premium/highest-resolution output. If they ask for draft/cheap/480p, keep the selected model and set `video_resolution: "480p"` when supported. For a non-NSFW direct 16-30 second request, choose Seedance 2.5 while the selector remains automatic. For any NSFW/adult-explicit video request, choose Wan 3.0 instead; this semantic route has higher priority than the duration route, analogous to choosing Qwen for NSFW image requests.
...
When the user explicitly selects Wan 3.0 or the request is NSFW/adult-explicit, use `model: "wan-3.0"` for MuleRouter Standard at 480p/720p/1080p. NSFW routing has priority over the 16-30 second Seedance 2.5 duration default. For explicit Wan 3.0 Prime, Fast, or lower-latency requests, use `model: "wan-3.0-prime"` for MuleRouter 480p/720p/1080p. For explicit Wan 3.0 Pro, super-resolution, 2K, or 4K, use `model: "wan-3.0-pro"` for MuleRouter 1080p/2K/4K. All three tiers support a single 2-30 second generation, native synchronized audio, and up to 10 image + 5 video + 5 audio feature references (20 total). With video references, the combined source duration and requested output duration must also stay within 30 seconds. Use `video_operation: "generate"`; Wan 3.0 does not expose typed edit/extend or a content-filter toggle in Makaron. Keep `seedance-fast` as the default for other requests.
...
- **Provider-wide image contract**: Treat every video-generation image as a feature reference by default, including a request with exactly one image. Never infer image-to-video or first-frame mode from the number of images. Such a workflow is valid only when a model capability and the caller both request it explicitly.
...
- **Wan 3.0 Standard / Prime / Pro**: MuleRouter generation routes for explicitly selected 2-30s text/image/multimodal videos. Standard (`wan-3.0`) supports 480p/720p/1080p; Prime (`wan-3.0-prime`) is the faster 480p/720p/1080p route; Pro (`wan-3.0-pro`) is the super-resolution route for 1080p/2K/4K. Use `<<<media_N>>>` and `<<<audio_N>>>`; Makaron translates them to Wan's `Image N`, `Video N`, and `Audio N` provider markers. Supports up to 10 images, 5 videos, and 5 audio files (20 total). It is reference generation, not direct video edit/extend, and has no Makaron content-filter switch.
