...
export function normalizeOutputs(raw: unknown, outputDir: string): MediaSandboxOutput[] {
  ...
  return rawOutputs
    ...
    .map(item => {
      // saveOutput() returns a durable output descriptor. Models sometimes put
      // that descriptor in `path` instead of spreading it into the returned
      // output. Accept both shapes so a successfully saved deterministic media
      // repair is not lost and needlessly regenerated by an image model.
      const nested = item.path && typeof item.path === 'object'
        ? item.path as Record<string, unknown>
        : undefined
      const localPath = typeof item.path === 'string'
        ? item.path
        : typeof nested?.path === 'string'
          ? nested.path
          : undefined
      return {
        path: localPath ? normalizeLocalPath(localPath, outputDir) : undefined,
        workspacePath: typeof item.workspacePath === 'string'
          ? item.workspacePath
          : typeof nested?.workspacePath === 'string'
            ? nested.workspacePath
            : undefined,
        storageUrl: typeof item.storageUrl === 'string'
          ? item.storageUrl
          : typeof nested?.storageUrl === 'string'
            ? nested.storageUrl
            : undefined,
        contentType: typeof item.contentType === 'string'
          ...
          : typeof item.mimeType === 'string'
            ? item.mimeType
            : typeof nested?.contentType === 'string'
              ? nested.contentType
              : undefined,
...
export async function runNodeMediaCode(options: RunNodeMediaCodeOptions): Promise<MediaSandboxResult> {
  ...
  try {
    ...
    const savedOutputsByWorkspacePath = new Map<string, {
      workspacePath: string;
      storageUrl?: string;
      contentType: string;
      size: number;
    ...
    const saveOutput = async (localPath: string, workspacePath?: string, contentType?: string) => {
      ...
      savedOutputsByWorkspacePath.set(outPath, output)
      return output
    }
    ...
    for (const output of outputs) {
      const relativeOutputPath = output.path && !existsSync(output.path)
        ? path.relative(outputDir, output.path)
        : undefined
      const previouslySaved = (output.path
        ? savedOutputsByLocalPath.get(path.resolve(output.path))
        : undefined)
        || (output.workspacePath
          ? savedOutputsByWorkspacePath.get(output.workspacePath)
          : undefined)
        || (relativeOutputPath && !relativeOutputPath.startsWith('..')
          ? savedOutputsByWorkspacePath.get(relativeOutputPath)
          : undefined)
