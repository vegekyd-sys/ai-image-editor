...
- Single-call total duration: Seedance 2.5 is 4-30 seconds; Wan 3.0 is 2-30 seconds; SeeDance 2.0 is 4-15 seconds; SeeDance/SeeDance Mini and MiniMax H3 are 4-15 seconds; Kling is 5-15 seconds; Grok 1.5 is 1-15 seconds; Google Omni is 3-10 seconds. For a non-NSFW direct 16-30 second request, choose Seedance 2.5 while the app selector remains automatic. For any NSFW/adult-explicit video request, default to Wan 3.0 Prime while the app selector remains automatic; this semantic route has higher priority than the duration route, analogous to choosing Qwen for NSFW image requests.
...
- Default model follows app selection, usually SeeDance 2.0 Fast (\`seedance-fast\`) at 720p; do not silently change it except for semantic capability routing. Use \`seedance-2.5\` when the user asks for Seedance 2.5, a non-NSFW single 16-30 second generation, more than the older reference limits, or its dedicated edit/extend features. The existing NSFW semantic route defaults to \`wan-3.0-prime\` and overrides the 16-30 second Seedance 2.5 route while the app selector is automatic; preserve an explicit user or app selection of \`wan-3.0\`. Resolution is one shared control for every video service: infer \`video_resolution\` from the full request and current app selection, use a value supported by the selected model, or keep its default when unspecified. Do not build provider-specific keyword routing for resolution. Wan exposes \`wan-3.0\` and \`wan-3.0-prime\`, both with 480p/720p/1080p/2K/4K. Both generate 2-30s with up to 10 image + 5 video + 5 audio references and do not expose typed edit/extend or a content-filter toggle. Use \`minimax-h3\` only when the selector or user explicitly asks for MiniMax/H3/Hailuo H3; it supports public 768p and native 2K multimodal generation, defaulting to 768p.
...
- The script must have been shown to the user and confirmed before this tool is called, unless the user's current request explicitly asks for direct submission without confirmation or the system prompt supplies the trusted Skill template launch exception.`,
      inputSchema: z.object({
        ...
        model: z.string().optional().describe('Video model/provider id. Supported ids include seedance-fast, seedance-mini, seedance, seedance-2.5, wan-3.0, wan-3.0-prime, kling, grok, google-omni, minimax-h3, and sync-lipsync-v3. Choose seedance-2.5 for non-NSFW direct 16-30s requests and default to wan-3.0-prime for the existing NSFW semantic route while the app selector is automatic. Use sync-lipsync-v3 only with exactly one source video and one replacement audio ref.'),
        video_resolution: z.enum(['360p', '480p', '720p', '768p', '1080p', '2k', '4k', 'auto']).optional().describe('Shared output-resolution control for every video model. Infer it from the complete user intent and current app selection, choose a value supported by the selected model, or use auto/default when unspecified. Grok 1.5 supports 480p/720p/native 1080p for text-to-video; any image/voice reference and video edit/extend are capped at 720p. Gemini Omni 1.1 supports 360p drafts, 720p native/default, and upscaled 1080p/4k.'),
