...
- Single-call total duration: Seedance 2.5 is 4-30 seconds; Wan 3.0 is 2-30 seconds; SeeDance 2.0 is 4-15 seconds; SeeDance/SeeDance Mini and MiniMax H3 are 4-15 seconds; Kling is 5-15 seconds; Grok 1.5 is 1-15 seconds; Google Omni is 3-10 seconds. Use the app-selected model; never switch to Wan 3.0 unless the user explicitly selected or requested it.
...
- Long source video rule: if a timeline/reference video is longer than the selected model's input limit (15 seconds for SeeDance 2.0 or MiniMax H3, 30 seconds for Seedance 2.5 or Wan 3.0), use \`skills/long-video-director/SKILL.md\`, analyze/split it into model-sized self-contained segments, and submit one script per segment after approval.
...
- Wan 3.0 reference limit: use generation mode with up to 10 images, 5 videos, and 5 audio files (20 total). Makaron accepts provider-readable MP4/MOV references up to 200MB each and enforces a 30-second combined input budget.
...
- Default model follows app selection, usually SeeDance 2.0 Fast (\`seedance-fast\`) at 720p; do not silently change it. Use \`seedance-2.5\` when the user asks for Seedance 2.5, a single 16-30 second generation, more than the older reference limits, or its dedicated edit/extend features. Use \`wan-3.0\` only when the selector or user explicitly asks for Wan 3.0; it is a lower-cost 2-30s Evolink route with 480p/720p/1080p output and up to 10 image + 5 video + 5 audio references, but it does not expose typed edit/extend or a content-filter toggle. Use \`minimax-h3\` only when the selector or user explicitly asks for MiniMax/H3/Hailuo H3; it supports public 768p and native 2K multimodal generation, defaulting to 768p. Use 2K only when explicitly requested or when the user asks for maximum/final quality.
- Grok modes: text-to-video and single-image-to-video support 480p/720p/native 1080p; 2-7 reference images support up to 720p. Single-image-to-video must omit \`aspect_ratio\` because xAI stretches the source when forced. Multi-reference prompts must map every image to a role. Optional \`reference_voice_ids\` accepts up to three xAI preset voices such as eve/leo; do not put uploaded audio URLs there.
...
- The script must have been shown to the user and confirmed before this tool is called, unless the user's current request explicitly asks for direct submission without confirmation or the system prompt supplies the trusted Skill template launch exception.`,
      inputSchema: z.object({
        story_prompt: z.string().describe('The complete video script. First line = short title, then the body. Native SeeDance, Wan 3.0, or MiniMax H3 text-to-video uses no media markers; Gemini Omni 1.1 follows the same rule. Makaron translates <<<media_N>>> / <<<audio_N>>> into each Evolink model family\'s provider markers.'),
        ...
        model: z.string().optional().describe('Video model/provider id. Supported ids include seedance-fast, seedance-mini, seedance, seedance-2.5, wan-3.0, kling, grok, google-omni, minimax-h3, and sync-lipsync-v3. Use sync-lipsync-v3 only with exactly one source video and one replacement audio ref.'),
        video_resolution: z.enum(['360p', '480p', '720p', '768p', '1080p', '2k', '4k', 'auto']).optional().describe('Output resolution. Grok 1.5 supports 480p/720p/native 1080p for text or one starting image; multi-image reference mode and video edit/extend are capped at 720p. Gemini Omni 1.1 supports 360p drafts, 720p native/default, and upscaled 1080p/4k.'),
      ...
      execute: async ({ story_prompt, duration, aspect_ratio, model, video_resolution, media_refs, audio_refs, reference_voice_ids, video_ref_url, video_ref_type, keep_original_sound, motion_control, character_orientation, video_operation, extend_direction, generate_audio, content_filter, output_format, web_search, completion_actions }) => serializeVideoSubmission(async () => {
        ...
        try {
          ...
          if (resolvedAudioRefs.audioUrls.length > 0 && videoRoute.provider !== 'seedance' && videoRoute.provider !== 'wan' && videoRoute.provider !== 'minimax' && videoRoute.provider !== 'fal-sync') {
            return {
          ...
          if (video_ref_url && autoVideoUrls.length > 0 && videoModel !== 'seedance-2.5' && videoModel !== 'wan-3.0') {
            return {
              success: false as const,
