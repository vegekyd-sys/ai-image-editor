...
- Default model follows app selection, usually SeeDance 2.0 Fast (\`seedance-fast\`) at 720p; do not silently change it except for semantic capability routing. Use \`seedance-2.5\` when the user asks for Seedance 2.5, a non-NSFW single 16-30 second generation, more than the older reference limits, or its dedicated edit/extend features. The existing NSFW semantic route uses \`wan-3.0\` and overrides the 16-30 second Seedance 2.5 route. Resolution is one shared control for every video service: infer \`video_resolution\` from the full request and current app selection, use a value supported by the selected model, or keep its default when unspecified. Do not build provider-specific keyword routing for resolution. Wan exposes \`wan-3.0\` and \`wan-3.0-prime\`, both with 480p/720p/1080p/2K/4K. Both generate 2-30s with up to 10 image + 5 video + 5 audio references and do not expose typed edit/extend or a content-filter toggle. Use \`minimax-h3\` only when the selector or user explicitly asks for MiniMax/H3/Hailuo H3; it supports public 768p and native 2K multimodal generation, defaulting to 768p.
