...
- FAL H3 Max (\`fal-h3-max\`) supports native text-to-video and image/video/audio reference-to-video at 480p/768p (default 768p), integer 5–15s. Use the same media/audio markers as other reference models. For modifying source video use video_intent="generate", video_operation="generate" and a feature video reference with precise edit instructions; do not request typed edit/extend or promise unchanged original audio. Maximum 9 images, 3 videos, 3 audios, 12 total; video/audio each 2–15s and each modality total <=15s.
- fal H3 Turbo exception: only when \`minimax-h3-max\` is selected, zero images means text-to-video and exactly one selected image means image-to-video from that start frame. Never pass more than one image, any video, or any uploaded audio. Default to native 768p; use 480p only when the user explicitly prioritizes the lowest cost or latency.
