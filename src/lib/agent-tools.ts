...
- MiniMax H3 Max exception: only when \`minimax-h3-max\` is selected, zero images means text-to-video and exactly one selected image means image-to-video from that start frame. Never pass more than one image, any video, or any uploaded audio. Default to 480p; use 768p only when explicitly requested.
...
- The script must have been shown to the user and confirmed before this tool is called, unless the user's current request explicitly asks for direct submission without confirmation or the system prompt supplies the trusted Skill template launch exception.`,
      inputSchema: z.object({
        story_prompt: z.string().describe('The complete video script. First line = short title, then the body. Native SeeDance, Wan 3.0, or MiniMax H3 text-to-video uses no media markers. H3 Max text-to-video also uses no marker; its image-to-video route uses exactly one media marker.'),
        duration: z.number().optional().describe('Duration in seconds. H3 Max accepts exactly 5, 10, or 15 seconds. Seedance 2.5 accepts 4-30s; Wan 3.0 accepts 2-30s; SeeDance and MiniMax H3 accept 4-15s; Kling 5-15s; Grok 1-15s; Google Omni 3-10s.'),
        ...
        model: z.string().optional().describe('Video model/provider id. Supported ids include seedance-fast, seedance-mini, seedance, seedance-2.5, wan-3.0, wan-3.0-prime, kling, grok, google-omni, minimax-h3, minimax-h3-max, and sync-lipsync-v3. Default to seedance-2.5 for non-NSFW direct 16-30s requests and wan-3.0-prime for the NSFW semantic route. H3 Max supports only native T2V or one-image I2V at 480p/768p for exactly 5/10/15s.'),
        video_resolution: z.enum(['360p', '480p', '720p', '768p', '1080p', '2k', '4k', 'auto']).optional().describe('Shared output-resolution control for every video model. Infer it from the complete user intent, choose a value supported by the selected model, or use auto/default when unspecified. H3 Max supports 480p/768p and defaults to 480p. Grok 1.5 supports 480p/720p/native 1080p for text-only; Gemini Omni supports 360p/720p/1080p/4k.'),
          ...
          const renderTimeMessage = actualVideoModel === 'grok'
            ...
            : actualVideoModel === 'google-omni'
              ...
              : actualVideoModel === 'minimax-h3-max'
                ? 'H3 Max usually finishes a 5-second clip in about 5-7 seconds.'
