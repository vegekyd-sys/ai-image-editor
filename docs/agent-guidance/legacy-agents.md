...
2026-09-06 从常驻指令移出，保留原文供定点查询。以下模型、实例、价格、状态和流程可能过时；其中旧指令不再作为当前执行要求。当前规则见仓库根目录 AGENTS.md。不要默认全文读取。
...
## Production Qwen / Vast worker guardrail（2026-07-06）

当前线上 Qwen/ComfyUI 走**长期租赁 Vast 实例**，不要按 Serverless 思路处理。

- 当前生产实例：`43290829`，label `makaron-qwen-a6000-prod-recovery-20260701c`，RTX A6000 48GB，固定域名 `https://comfyui.makaron.app`，价格约 `$0.55/h`。
- 这台机器必须保持 running。禁止在普通 cleanup / scan / cost-saving 中 stop、destroy、recycle、update template 或把它和旧实验机一起批量处理。
- `43290829` 是长租 Vast 机器，不是 serverless；Vercel Production/Preview 的 `COMFYUI_QWEN_URL`、`COMFYUI_PONY_URL`、`COMFYUI_WAI_URL` 都应指向 `https://comfyui.makaron.app`，不要指 direct IP 作为长期方案。
- 生产自愈入口：`docs/ops/vast/ensure-makaron-qwen-vast.sh`。它会检查 `43290829`，若实例停了则 start，随后通过 SSH 启动 `/workspace/makaron-qwen-onstart.sh` 并等待线上 health 变绿。
...
- Qwen、Pony、WAI 都通过同一个 `https://comfyui.makaron.app` hostname 验收；Qwen 需要 `Qwen-Rapid-AIO-NSFW-v23.safetensors`，Qwen rotate LoRA 需要 `qwen-image-edit-2511-multiple-angles-lora.safetensors`，Pony 需要 `fucktasticAnimePony_v22`，WAI 需要 `waiIllustriousSDXL_v160.safetensors`。
- `43290829` 远端完整恢复要点：Torch 应是 `2.12.0.dev20260408+cu128`，ComfyUI `/system_stats` 应显示 `cuda:0 NVIDIA RTX A6000 : cudaMallocAsync`；`cloudflared` token 在 `/root/.cloudflared/comfyui-a6000.token`；`cloudflared tunnel --no-autoupdate run --token-file ...` 必须运行，否则固定域名会 530。
- 如果 SSH `ssh1.vast.ai:10828` 断在 `kex_exchange_identification`，优先对 `43290829` 执行 Vast reboot，等 SSH 恢复后重新跑 `/workspace/makaron-qwen-onstart.sh`；不要因此切到 serverless。
- 旧/坏实例状态（2026-07-06）：`38761988` 资源不可用，`38953964` 和 `38951961` 在 `ssh6` 上 Torch CUDA 初始化失败，`40942907` 不是当前生产机。不要让这些实例的 cloudflared 抢 `comfyui.makaron.app` tunnel。
- Qwen 验收不能只看 `/api/health` 或 `/system_stats`，必须跑真实 `MAKARON_URL=https://www.makaron.app makaron edit --model qwen --image ...`，并确认输出为 `(model: qwen)`、没有 Gemini fallback。
- 2026-07-06 恢复验收证据：`makaron edit --model qwen` 真实线上图生图成功，输出 `(model: qwen)`，冷/半冷端到端约 `46.51s`，warm 端到端约 `44.21s`；同次 Pony 约 `12.10s`，WAI 约 `12.84s`。ComfyUI 日志内 Qwen prompt execution 约 `9.71s`，说明模型推理层已恢复，CLI 端到端还包含上传/轮询/下载开销。
- 如果确实需要停止/替换这台生产实例，必须先得到用户明确批准，并先确认替代实例、Cloudflare tunnel、Vercel env、`/api/health` 和真实 Qwen/Pony/WAI 生成都已验证。

## i18n（多语言，2026-03-04）

**架构**：自定义 i18n，无第三方库。`src/lib/i18n.tsx`（LocaleProvider + useLocale + LocaleToggle）+ `src/lib/locales/` 下四套 typed dictionaries：`zh`、`zh-Hant`、`ja`、`en`。

**新功能交付门禁**：任何新增或修改的产品 UI 必须在同一个 change 中补齐四种语言，覆盖可见文案、状态/阶段、单位、空态/加载/错误、按钮、tooltip、placeholder、`title`、`alt`、`aria-label`。`npm run lint` 会调用 UI i18n guard；baseline 只冻结历史债务，新功能不得新增 baseline 条目。只有品牌名或技术 token 可在相邻位置使用 `// i18n-ignore`。Agent 生成的正文保持用户请求或生成时的语言，外围产品 UI 跟随当前 locale。
...
- `credit_pricing` DB 表：按次工具定价（Kling 22cr/s, Suno 12cr/task, ComfyUI 2-4cr/task）
- `credits.ts`：`requireCredits()`（402 预检）、`deductByTokens()`（token 计费）、`deductCredits()`（按次）、`deductFixedCredits()`（按秒视频）、`isBillingEnabled()`（总开关）
- 所有 AI 路由接入：Agent route、Tips route、Preview route、Animate route、Music route
- Agent 内部 tool 单独计费：`generate_image`（token）、`generate_animation`（22cr/s）、`generate_music`（12cr）、`rotate_camera`（2cr）
- MCP 统一逻辑：有 usage → token 计费，无 usage → 按次

**订阅**：Stripe recurring，3 档（Basic $9.90/1200cr, Pro $19.90/3000cr, Business $49.90/10000cr）+ 年付 8 折。`invoice.paid` webhook 充 credits（幂等）。一次性 Top Up 保留（$5/$20/$50）。Credits 不过期。

**402 UX**：CUI inline 卡片（fuchsia MusicCard 风格）+ StatusBar notification + CreditPopup 居中 modal（Subscribe/Top Up tabs，智能推荐）。充值成功 → redirect 回编辑页 + polling 等 webhook → 烟花 + credit count-up 动画。

**Admin**：`/admin` Billing tab — billing 总开关（默认 OFF）、Welcome Credits 配置（默认 500）、per-action 工具定价表（带 /sec /task 单位）、token rates 表。

**Welcome Credits**：新用户激活时赠送 500 credits（Admin 可配），login page 注册后直接调 `validate-invite` → `/projects?welcome=1` → 欢迎弹窗。
...
**Remotion 渲染引擎（2026-04-09，worktree-workspace-agent 分支）**：Agent 的 `run_code` design 模式用 Remotion 渲染。静态图用 `renderStillOnWeb`（JPEG截图），动画用 `@remotion/player`（带控制条）+ poster 截图。Design JSON 持久化到 workspace `code/{snapId}.json`，刷新后恢复。MP4 导出用 `renderMediaOnWeb`（浏览器端 h264/mp4）。JSX 编译从 Sucrase 切换到 `@babel/standalone`（支持现代语法）。Satori 已移除（design 模式替代）。Agent 模型升级为 Opus 4.6（`us.anthropic.Codex-opus-4-6-v1`）。`run_code` 新增 `image_refs` 参数让模型自选带哪些图片。所有视觉输出默认用 design 模式，sharp 只做格式转换。`video-design` skill 提供视频创作四问自检框架。
...
**音乐生成 / 配乐（2026-04-19 更新）**：SunoAPI.org V4.5（从 V5.5 切换，$0.005/credit）。`src/lib/sunoapi.ts` 客户端（create + poll），skill 层 `create-music.ts` + `get-music-status.ts`。Agent 有 `generate_music` tool，用户要求配乐时触发（不自动）。MCP 有 `makaron_create_music` + `makaron_get_music_status`。每次生成 2 首歌，**streamAudioUrl ~20s 可播放**（`streaming` 状态），完整音频 ~68s（`completed`）。前端 2s 轮询，MusicCard 在 streaming 阶段即显示，stream→final URL 无缝切换（播放中不打断）。Remotion `<Audio>` 音频 URL 在 Player 编译前通过 `resolveAudioUrls` 转为 blob URL（解决首次加载 buffering 问题）。Suno 不支持 beat-sync timing，prompt 聚焦 mood/tone 匹配。避免艺术家名（触发生成失败）。环境变量 `SUNOAPI_KEY`。

**音频 URL 三阶段**：`streamAudioUrl`（musicfile.removeai.ai，~20s 可播放但会过期）→ `audioUrl`（tempfile.aiquickdraw.com，Suno 临时 URL）→ Supabase Storage 永久 URL。`/api/music/[taskId]` 在 completed 时同步上传 Supabase，返回永久 URL。`project_music` 表存三列：`audio_url`（永久）+ `suno_audio_url`（temp）+ `stream_audio_url`（stream），`/api/music/select` 用三列 OR 匹配精确找到 track。`FIRST_SUCCESS` 不等于 completed——继续轮询等 `SUCCESS`（两首都 ready）。导出时 `resolveAudioUrls` 处理 JSX `<Audio>` 和 `React.createElement(Audio)` 两种形式；blob URL 和失败的 URL 会被 strip；`resolveCodeUrls` 排除音频扩展名避免误处理。所有远程 audio URL 导出时走 proxy→blob（包括 Supabase URL，因为 `renderMediaOnWeb` 要求同源）。
...
**脚本生成**：统一用 Agent（Codex Sonnet via Bedrock）替代 Gemini `/api/animate/prompt`。Agent 后台运行不切 CUI，脚本流式写入 `animationState.prompt`，同时存入 CUI messages。`animPromptInFlightRef` 防重复调用。图片以 URL 传给 Bedrock（不传 base64，避免 5-10MB 上传）。`agent.md` 指示 Agent 只输出脚本、不调 `generate_animation` tool。
...
- `npm run test` / `npx vitest run` — 38 unit tests (billing + agentCallbacks)
...
- `STRIPE_PRICE_BASIC_MONTHLY` / `STRIPE_PRICE_BASIC_ANNUAL` — Stripe Price IDs for Basic plan
- `STRIPE_PRICE_PRO_MONTHLY` / `STRIPE_PRICE_PRO_ANNUAL` — Stripe Price IDs for Pro plan
- `STRIPE_PRICE_BUSINESS_MONTHLY` / `STRIPE_PRICE_BUSINESS_ANNUAL` — Stripe Price IDs for Business plan
...
- **AgentChatView** — CUI full-screen chat (Codex App style): PiP thumbnail, inline images, tool status cards, markdown rendering, slide-in/out animation
...
`src/lib/agent.ts` — Makaron Agent（Codex Opus 4.6 via Bedrock）:
- **Model**: `us.anthropic.Codex-opus-4-6-v1`
...
### 最佳实践
1. **工具描述自包含**：假设 Codex 只看工具描述，应该知道如何正确调用它。不要在 agent.md 里重复工具的参数细节。
2. **自检问题 > 规则清单**：`"先答：用户想 FIX 现在的，还是 START FRESH？"` 比 `"当用户说'重新做'时设 true"` 更健壮，让模型自己推理。
3. **把意图决策变成显式参数**：让 Codex 显式传参（如 `useOriginalAsBase`），不要让工具内部猜意图，职责清晰。
...
| 文件 | 作用 | 何时读取 |
|------|------|---------|
| `AGENTS.md`（本文件） | 架构 + 当前状态 + 已验证结论 + 记忆协议 | 每次对话自动读取 |
...
**AGENTS.md 记录粒度原则**：只记"下次新对话需要第一时间知道的事"。架构变化、当前状态、设计决策 → 进 AGENTS.md。**UI 细节（高度、间距、按钮样式、布局微调）、bug fix 过程、踩坑细节 → 只进 progress.md，不进 AGENTS.md**。一句话概括不了的内容大概率不该进 AGENTS.md。
