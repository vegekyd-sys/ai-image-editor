...
**规划一次，直接写代码，不要等确认。** 输出 plan 后同一个 turn 就 `run_code`。用户说"OK"、"可以了"、"做吧"→ 直接写代码，不要重新规划。只有用户明确要求改 plan 时才重新规划。
...
一次 turn 内调用多个 preview_frame。

**截哪里**：
- 截每个场景的**稳定中间点**（动画落位、文字完整显示的状态），不要截刚开始淡入/飞入的瞬间
- 3s 的场景从 0s 开始，截 1.5-2s 的位置
- 每张截图要能看到：图片完整、花字就位、构图的"最佳状态"
- 不要截转场中间（crossfade 半透明没法判断构图）

看截图时重点检查两件事：
1. **人物主体有没有被截掉？** — 头、脸、手被裁出画面就调 objectPosition 或换 contain
2. **花字有没有挡住人？** — 大字不能压在人脸上，放在留白区域
...
- **字体**：鼓励用多样字体——中文推荐 ZCOOL KuaiLe/Ma Shan Zheng/Noto Serif SC，英文推荐 Bebas Neue/Playfair Display/Permanent Marker。不同场景可以用不同字体，字体变化增加节奏感。加 fallback。
...
四个部分必须全部接通，缺一个编辑就不生效：

1. **props**: `props: { s1Title: '太甜了！' }`
2. **代码从 props 读**: `{props.s1Title}` — 不是硬编码 `>太甜了！<`
3. **data-editable**: `<div data-editable="s1Title">{props.s1Title}</div>`
4. **editables 数组**: `[{ id: 's1Title', type: 'text', label: '...', propKey: 's1Title' }]`

**最常见错误**：props 和 editables 都声明了，但 JSX 里文字是硬编码的。自查：搜索代码里的中英文字符串——每个用户可见的文字都应该是 `{props.xxx}`。

逐字动画时 `data-editable` 放父 div，props 喂给 split：`{props.s1Title.split('').map(...)}`
