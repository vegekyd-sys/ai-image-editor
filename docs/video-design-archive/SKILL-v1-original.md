...
### 阶段 1.5：图片适配
规划时考虑每张图片的实际尺寸和比例（横版 4:3？竖版 9:16？正方形？），决定画面怎么放：
- 横版照片在竖版画布（1080×1920）完全可以：上下留空区域用来放花字、渐变、装饰元素，让画面整体好看
- 多张照片比例不同：用 `objectFit: 'cover'` + `objectPosition` 控制焦点（人脸居中），或根据构图需要用 `contain` 配背景
- **绝对保留主体人物**——`objectPosition` 对准人脸/人物中心，宁可裁背景不要裁人
- 关键：**画面整体构图要好看**——图片、花字、装饰元素作为一个整体设计，不是简单铺满
