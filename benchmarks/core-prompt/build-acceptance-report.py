...
def add(case, variant, repeat, p, kind, probe=None, model=None):
    ...
    row=groups.setdefault(key,dict(id=key,case=case,group=group,title=title,criteria=criteria,repeat=repeat,source=sources.get(source),reference=sources.get('reference-face.jpg') if case=='accept-image-restore' else sources.get('poster.jpg') if case=='multi' else None,items={}))
...
<section id="review"><span class="eyebrow">BLIND REVIEW</span><h2>先看作品，再揭晓版本</h2><p>原始素材单独展示；A、B 是两版系统对同一要求各自生成的结果，两张都已经编辑过。版本位置随机打乱，评分后可揭晓。第1、2、3轮是同一案例的三次重复测试。评分保存在当前浏览器。</p><div class="filters"><select id="category" aria-label="选择类别"><option>全部</option><option>图片</option><option>视频</option><option>编码</option></select><select id="case" aria-label="选择案例"></select><button id="prevTop">上一组</button><button id="nextTop">下一组</button></div><div id="content"></div><div class="votes"><button data-vote="A">A 更好</button><button data-vote="equal">差不多</button><button data-vote="B">B 更好</button><button data-vote="both_bad">都没达标</button></div><div id="result" role="status"></div><div class="controls"><button id="prev">上一组</button><button id="next">下一组</button><button id="copy">复制评分</button><button id="download">下载评分</button></div><p id="progress" class="muted"></p><textarea id="export" hidden readonly aria-label="评分结果，可手动复制"></textarea></section>
...
const media=x=>!x.src?'<div class="missing">本轮无可比输出：路由未通过或只完成了前置步骤。未用其他样本替补。</div>':x.kind==='video'?`<video controls playsinline preload="none" src="${esc(x.src)}"></video>`:`<a href="${esc(x.src)}" target="_blank" rel="noopener"><img loading="lazy" src="${esc(x.src)}" alt="图片，点击查看完整尺寸"></a>`;
function render(){const r=visible[current];if(!r)return;$('#case').value=r.id;history.replaceState(null,'','#review-'+r.id);$('#content').innerHTML=`<h2>${esc(r.title)} · 第${r.repeat+1}轮</h2><p>${esc(r.criteria)}</p>${r.source?`<div class="source"><figure><h3>原始素材 · 编辑前</h3>${media({src:r.source,kind:r.source.endsWith('.mp4')?'video':'image'})}</figure>${r.reference?`<figure><h3>第二份参考素材</h3>${media({src:r.reference,kind:'image'})}</figure>`:''}</div>`:''}<div class="pair">${r.items.map((x,i)=>`<figure><figcaption>${i?'B':'A'} · 生成结果<span class="result-context">${esc(r.title)} · 第${r.repeat+1}轮<br>${esc(r.criteria)}</span></figcaption>${media(x)}</figure>`).join('')}</div><details><summary>揭晓本组版本与文件信息</summary>${r.items.map((x,i)=>`<p>${i?'B':'A'} = ${x.version==='candidate'?'重构候选':'旧版'}${x.model?' · '+esc(x.model):''}${x.probe?' · '+esc(x.probe.width)+'×'+esc(x.probe.height)+(x.probe.duration?' · '+esc(x.probe.duration)+'秒':''):''}</p>`).join('')}</details>`;$('#prev').disabled=$('#prevTop').disabled=current===0;$('#next').disabled=$('#nextTop').disabled=current===visible.length-1;$('#result').textContent=votes[r.id]?'已保存：'+({A:'A 更好',B:'B 更好',equal:'差不多',both_bad:'都没达标'}[votes[r.id]]):'尚未评分';document.querySelectorAll('[data-vote]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.vote===votes[r.id])));$('#progress').textContent=`当前类别 ${current+1}/${visible.length} 组 · 全部已评 ${Object.keys(votes).length}/${pairs.length} 组`}
...
$('#category').onchange=filter;$('#case').onchange=()=>{current=visible.findIndex(r=>r.id===$('#case').value);render()};$('#prev').onclick=$('#prevTop').onclick=()=>{current--;render();$('#content').scrollIntoView({behavior:'smooth'})};$('#next').onclick=$('#nextTop').onclick=()=>{current++;render();$('#content').scrollIntoView({behavior:'smooth'})};document.querySelectorAll('[data-vote]').forEach(b=>b.onclick=()=>{votes[visible[current].id]=b.dataset.vote;try{localStorage.setItem('core-acceptance-v2',JSON.stringify(votes))}catch{}render()});
