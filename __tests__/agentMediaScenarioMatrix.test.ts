...
describe('agent media scenario matrix', () => {
  ...
  it('separates generic coding, Remotion composition, and node media outputs', () => {
    ...
    expect(coding).toContain('For the full text/image/video/trim rules, read `prompts/remotion-composition.md`')
    ...
    expect(remotion).toContain('canonical editable contract for Remotion compositions')
    expect(remotion).toContain('Do not wait for the user to say "editable"')
    expect(remotion).toContain('scene years, titles, subtitles, captions, badges, counters, stats, brand names')
    expect(remotion).toContain('Before returning, do an editable coverage check')
    expect(remotion).toContain('store prop keys in the array, not the text itself')
    expect(remotion).toContain('Never use an empty editable id')
    expect(remotion).toContain('Every visible image card/layer should have its own stable editable id')
    expect(remotion).toContain('complete `props` and `editables` arrays in the first part')
    expect(remotion).toContain('accepts `text`, `image`, and `video`')
    expect(remotion).toContain('Low-burden text pattern for multi-scene compositions')
    expect(remotion).toContain('`data-editable="fieldId"` sits on the visible measurable wrapper')
    expect(remotion).toContain('Primary image and video layers')
    expect(remotion).toContain('Video trim pattern')
    expect(remotion).toContain('trimBeforePropKey')
    ...
    expect(remotion).toContain('put the literal 1-based `<<<media_N>>>` marker in props or code')
    expect(remotion).toContain('Never manually translate Media Index N to `ctx.snapshotImages[N]`')
  ...
  it('keeps agent-visible media context on composition terminology', () => {
    ...
    expect(agentTs).toContain('Capture one visual frame or a 2-6 frame contact sheet')
    ...
    expect(agentTs).toContain('autosaved to')
    expect(agentTs).toContain('code_path: autosave.path')
    expect(agentTs).not.toContain('Draft is not saved yet')
    expect(agentTs).toContain('persistCompositionDraft')
    ...
    expect(designHarness).toContain('a Media Index marker could not be resolved')
