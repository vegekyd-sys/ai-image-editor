...
describe('agent media scenario matrix', () => {
  ...
  it('separates generic coding, Remotion composition, and node media outputs', () => {
    ...
    expect(agentTs).toContain('Never save raw Remotion composition source via content/path')
  ...
  it('keeps agent-visible media context on composition terminology', () => {
    ...
    expect(agentTs).toContain('not published')
    expect(agentTs).toContain('If the user asked to publish or put it on the timeline')
    expect(agentTs).toContain('the next render event has published=true')
