...
describe('video generation intent and one-submit regression', () => {
  ...
  it('animates only the prepared frame in a multi-image H3 Max project', async () => {
    ...
    expect(readFileSync('src/lib/prompts/video-submission.md', 'utf8')).toContain('Multiple supplied images do not by themselves make fal H3 Turbo unusable');
  });
  ...
  it('does not let a stray contract change an ordinary model or resolution', async () => {
    ...
    expect(h.createVideo.mock.calls[0][0]).toMatchObject({ videoModel: 'fal-h3-max', videoResolution: '768p' });
  });
