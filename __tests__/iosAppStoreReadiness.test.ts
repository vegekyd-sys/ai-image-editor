...
describe('iOS App Store readiness guardrails', () => {
  ...
  it('gives secondary iOS app pages safe top space and a normal edge-back gesture', () => {
    ...
    expect(topBar).toContain('requestNativePageStackPush(path)');
    expect(topBar).not.toContain('const isPrimaryTabSwitch');
    expect(topBar).not.toContain('if (!isPrimaryTabSwitch) requestNativePageStackPush(path)');
    expect(topBar).not.toContain('primeNativeNavigation');
    expect(topBar).not.toContain("onTouchStart={() => primeNativeNavigation('/projects')}");
    ...
    expect(pageStack).toContain('isPrimaryNativeTabPath');
    expect(pageStack).toContain('pauseMediaForIOSStack(activeEntryRef.current)');
    expect(pageStack).toContain('keepLivePrimaryTab ?');
    expect(pageStack).toContain('function PendingProjectsShell');
    expect(pageStack).toContain('getLastProjectsListSync');
    expect(pageStack).toContain('PROJECTS_LIST_CACHE_UPDATED_EVENT');
    expect(pageStack).toContain('data-makaron-ios-pending-projects-shell');
    expect(pageStack).toContain('one man branding studio');
    expect(pageStack).toContain('getOriginFormatThumbnailUrl');
    expect(pageStack).toContain('makaron-ios-project-shimmer');
    ...
    expect(home).toContain('const revealHomeBehindSkillPan');
    expect(home).toContain("display: viewMode === 'agent' || (selectedDetail && !revealHomeBehindSkillPan) ? 'none' : undefined");
    ...
    expect(home).toContain("window.addEventListener('makaron-keyboard-inset-change'");
    ...
    expect(projects).toContain('aria-label="Loading projects"');
    expect(projects).toContain('mkr-shimmer');
    ...
    expect(imageCache).toContain('PROJECTS_LIST_CACHE_UPDATED_EVENT');
    expect(imageCache).toContain('window.dispatchEvent(new CustomEvent(PROJECTS_LIST_CACHE_UPDATED_EVENT');
  ...
  it('keeps iOS home skill detail back swipe scoped to the skill overlay', () => {
    ...
    expect(homePage).toContain('muted={variant !== \'detail\'}');
    expect(homePage).toContain('video.muted = false');
  ...
  it('keeps iOS project navigation inside a live projects-page overlay while preserving CUI pan close', () => {
    ...
    const globals = fs.readFileSync(path.join(root, 'src/app/globals.css'), 'utf8');
    const chatView = fs.readFileSync(path.join(root, 'src/components/AgentChatView.tsx'), 'utf8');
    const videoResultCard = fs.readFileSync(path.join(root, 'src/components/VideoResultCard.tsx'), 'utf8');
    const imageRefChip = fs.readFileSync(path.join(root, 'src/components/ImageRefChip.tsx'), 'utf8');
    const bridge = fs.readFileSync(path.join(root, 'ios/App/App/MakaronBridgeViewController.swift'), 'utf8');
    ...
    expect(editor).toContain('normalizeInlineImageIdentity');
    expect(editor).toContain('s.messageId === messageId');
    expect(editor).toContain('[data-makaron-cui-tap-target="true"]');
    expect(editor).toContain('[data-makaron-editor-tap-target="true"]');
    expect(editor).toContain('data-makaron-hero-overlay="true"');
    expect(projectsPage).toContain('isIOSProjectPanEditableTarget');
    expect(projectsPage).toContain('[data-makaron-editor-tap-target="true"]');
    expect(projectsPage).toContain('data-makaron-ios-project-pan-edge="true"');
    expect(projectsPage).toContain("top: 'calc(env(safe-area-inset-top) + 72px)'");
    ...
    expect(editor).toContain('onNavigateToSnapshot={handleNavigateToSnapshot}');
    expect(editor).not.toContain('onNavigateToSnapshot={undefined}');
    expect(imageRefChip).toContain("onTouchStart={() => { isTouchDevice.current = true; }}");
    expect(imageRefChip).not.toContain('showTouchPreview');
    expect(imageRefChip).toContain("getThumbnailUrl(imgSrc, 96, 85, 96, 'cover')");
    expect(imageRefChip).toContain("getThumbnailUrl(imgSrc, 680, 90, 680, 'contain')");
    expect(imageRefChip).toContain('setShowPreview(true)');
    expect(imageRefChip).not.toContain('aria-label="Open image on canvas"');
    expect(imageRefChip).not.toContain('navigateToSnapshot');
    expect(videoResultCard).toContain('data-makaron-editor-tap-target="true"');
