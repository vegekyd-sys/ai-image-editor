...
describe('external Media List publishing', () => {
  it('refreshes an existing durable range with richer media understanding', async () => {
    ...
    const existing = [{
      ...
      video_meta: {
        ...
        taskId: null,
        videoUrl: 'https://cdn.example.com/source.mp4?sig=old',
        providerUrl: 'https://cdn.example.com/source.mp4?sig=old',
        prompt: 'Old short label',
        sourceSnapshotIds: [],
        sourceUrls: ['https://cdn.example.com/source.mp4?sig=old'],
        sourceRange: {
          source_url: 'https://cdn.example.com/source.mp4?sig=old',
          source_uri: 'asset://factory/racket',
          start_sec: 12,
          end_sec: 18,
        },
        status: 'completed',
    ...
    const [published] = await publishExternalVideoRanges({
      ...
      projectId: 'project-1',
      ranges: [{
        source_url: 'https://cdn.example.com/source.mp4?sig=new',
        source_uri: 'asset://factory/racket',
        start_sec: 12,
        end_sec: 18,
        description,
    ...
    expect(updates[0].video_meta).toMatchObject({
      videoUrl: 'https://cdn.example.com/source.mp4?sig=new',
      prompt: description,
      sourceRange: {
        source_url: 'https://cdn.example.com/source.mp4?sig=new',
        source_uri: 'asset://factory/racket',
