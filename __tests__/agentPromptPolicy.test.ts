...
describe('agent prompt policy guards', () => {
  ...
  it('keeps single video generation in the model duration range and routes longer requests to the director skill', () => {
    ...
    expect(agent).toContain('a single SeeDance script/call must be 4-15s')
    expect(agent).toContain('If requested/source duration is shorter than the model minimum, use the minimum')
    expect(agent).toContain('If under 4s, set `duration: 4`')
    expect(agent).toContain('if a complete approved script is <=15s')
    expect(agent).toContain('submit it as one `generate_animation` call')
    expect(agent).toContain('Do not submit only one shot or split just because it has multiple shot lines')
    expect(agent).toContain('If output is longer than 15s')
    expect(agent).toContain('use `skills/long-video-director/SKILL.md`')
    expect(agent).toContain('[Active skill: long-video-director]')
    expect(agent).toContain('if an existing timeline/reference video is >15s')
    expect(agent).toContain('do not compress the whole source into one short edit')
    expect(agent).toContain('up to 15s combined source/reference video duration')
  ...
  it('routes long video work through the long-video-director skill before short video scripts', () => {
    ...
    expect(skill).toContain('prompts/animate.md` for final segment scripts')
    expect(agent).toContain('That workflow is staged review: story')
    ...
    expect(skill).toContain('Check that every segment has one approved storyboard image')
