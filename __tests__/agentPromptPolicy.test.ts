...
describe('agent prompt policy guards', () => {
  ...
  it('preserves full audio tool capabilities with a native-audio exception for direct video', () => {
    ...
    expect(agentTs).toContain("validateAudioKindForRequest(ctx.userRequest || '', kind)")
    expect(agentTs).toContain("requestsImageConditionedAudio(ctx.userRequest || '')")
    expect(agentTs).toContain('Never inherit the current or selected media automatically')
