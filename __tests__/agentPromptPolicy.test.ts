...
describe('agent prompt policy guards', () => {
  ...
  it('keeps single video generation in the model duration range and routes longer requests to the director skill', () => {
    ...
    expect(agent).toContain('one SeeDance generation may use up to 15s combined source/reference video duration')
  ...
  it('routes long video work through the long-video-director skill before short video scripts', () => {
    ...
    expect(agent).toContain('anchors')
    expect(agent).toContain('director beat board')
    expect(agent).toContain('storyboard image per segment')
    expect(agent).toContain('scripts/preflight')
