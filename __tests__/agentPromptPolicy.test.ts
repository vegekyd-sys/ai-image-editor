...
describe('agent prompt policy guards', () => {
  ...
  it('routes video by duration before activating semantic production skills', () => {
    ...
    expect(agent).toContain('Video routes by duration first')
    expect(agent).toContain('<=15s uses the default Agent')
    expect(agent).toContain('>15s may activate a matching Skill')
