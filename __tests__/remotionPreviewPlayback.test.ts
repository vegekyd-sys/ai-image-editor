...
describe('Remotion preview playback contract', () => {
  it('keeps the media runtime that supports Remotion playback and export', () => {
    ...
    expect(rendererSource).toContain('playerRef.current?.pause()')
    expect(rendererSource).toContain('const setPlayerRef = useCallback')
    expect(rendererSource).toContain('ref={setPlayerRef}')
    expect(rendererSource).toContain('publishes the Player synchronously')
    ...
    expect(evalSource).toContain('video.currentTime + 1 / (fps * 4) < minimumRevealTime')
    expect(evalSource).toContain('() => getPreviewMediaProxySource(src)')
    expect(evalSource).toContain("type PreviewVideoSourcePhase = 'direct' | 'proxy' | 'direct-retry'")
    expect(evalSource).toContain("const previewSrc = sourcePhase === 'proxy' ? proxyFallbackSrc : src")
    expect(evalSource).toContain('advanceSourceFallback()')
    expect(evalSource).toContain("current === 'direct' ? 'proxy' : 'direct-retry'")
    expect(evalSource).toContain("sourcePhase === 'direct-retry'")
    expect(evalSource).not.toContain('scheduleMediaRetry()')
    expect(evalSource).not.toContain('video.load()')
    expect(evalSource).toContain('(containsVideo && (props.from ?? 0) > 0)')
    expect(evalSource).toContain('authoredDuration + continuityFrames')
    expect(evalSource).toContain('Math.max(2, Math.round(fps / 10))')
    expect(evalSource).toContain('getPreviewPremountFrames({')
    expect(evalSource).toContain('iosWebKit: isIOSWebKit()')
    expect(evalSource).toContain('premountFor,')
    ...
    expect(evalSource).toContain('pauseWhenBuffering: !safari')
    expect(evalSource).toContain('const { delayPlayback } = Remotion.useBufferState()')
    expect(evalSource).toContain('safariBufferHandleRef.current = delayPlayback()')
    expect(evalSource).toContain('safariBufferMutedStateRef.current = video.muted')
    expect(evalSource).toContain('void video.play().catch(() => undefined)')
    expect(evalSource).toContain('markReadyRef.current(video)')
    expect(evalSource).toContain('const primeTrimStart = () => {')
    expect(evalSource).toContain('video.currentTime = targetTime')
    expect(evalSource).toContain('Seek hidden')
    expect(evalSource).toContain('const readinessPoll = window.setInterval')
    expect(evalSource).toContain('window.clearInterval(readinessPoll)')
    expect(evalSource).toContain('isPreviewMediaVisuallyActive(video)')
    expect(evalSource).toContain("element.classList.contains('makaron-preview-readiness-gate')")
    expect(evalSource).toContain("!isReadinessGate && Number.parseFloat(style.opacity || '1') <= 0.001")
    expect(evalSource).toContain('never block while the Player is still')
    expect(evalSource).toContain('if (!reportedReady.current)')
    expect(evalSource).toContain('event.currentTarget.currentTime = targetTime')
    expect(evalSource).toContain('(!event.currentTarget.paused || safariBufferHandleRef.current)')
    expect(evalSource).toContain('readiness?.isActiveMediaScene ?? frame >= 0')
    expect(evalSource).toContain('(parentReadiness?.isActiveMediaScene ?? true) && isWithinAuthoredRange')
    expect(evalSource).toContain("preload: preload ?? 'auto'")
    expect(evalSource).toContain("parsed.hostname === 'cdn.makaron.app'")
    expect(evalSource).toContain("parsed.hostname.endsWith('.supabase.co')")
    expect(evalSource).toContain('src: previewSrc')
    expect(evalSource).toContain('onWaiting: handleWaiting')
    expect(evalSource).toContain('onCanPlay: handleCanPlay')
    expect(evalSource).toContain('video.readyState < HTMLMediaElement.HAVE_FUTURE_DATA')
    expect(evalSource).toContain('Keep the timeline on the last valid frame')
    ...
    expect(evalSource).toContain('display: readiness?.showLastFrame && hasCachedFrame')
    ...
    expect(rendererSource).toContain('const videoResolved = design.code')
    expect(rendererSource).not.toContain('function resolvePreviewVideoUrls')
  ...
  it('does not auto-play Remotion compositions when code generation publishes a draft', () => {
    ...
    expect(source).toContain('pendingRemotionPlayRef.current = true')
    expect(source).toContain('if (ref && pendingRemotionPlayRef.current)')
    expect(source).toContain('start as soon as that ref is')
  })
  ...
  it('lets Remotion own buffering without explicitly pausing the player', () => {
    ...
    expect(source).toContain('onClick={(e) => { e.stopPropagation(); toggleRemotionPlay(); }}')
    expect(source).not.toContain('if (!remotionBuffering && !remotionLoading) toggleRemotionPlay()')
  })
