...
describe('Remotion preview playback contract', () => {
  it('keeps the media runtime that supports Remotion playback and export', () => {
    ...
    expect(evalSource).toContain('const previewSrc = useProxyFallback ? proxyFallbackSrc : src')
    expect(evalSource).toContain('scheduleProxyFallback()')
    expect(evalSource).toContain('switchToProxyFallback()')
    expect(evalSource).toContain('}, 2500)')
