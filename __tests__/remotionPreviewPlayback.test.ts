...
describe('Remotion preview playback contract', () => {
  it('keeps the media runtime that supports Remotion playback and export', () => {
    ...
    expect(evalSource).toContain('() => safari ? getPreviewMediaProxySource(src) : src')
    expect(evalSource).toContain('const previewSrc = useSafariProxyFallback ? proxyFallbackSrc : src')
    expect(evalSource).toContain('scheduleSafariProxyFallback()')
    expect(evalSource).toContain('switchToSafariProxyFallback()')
    ...
    expect(evalSource).toContain('fps * (isSafariWebKit() ? 6 : 3)')
