...
describe('Remotion preview playback contract', () => {
  it('keeps the media runtime that supports Remotion playback and export', () => {
    ...
    expect(rendererSource).toContain('loadRemotionGoogleFonts(remotionFontSearchText(fontSafeCode, resolvedProps))')
    expect(rendererSource).toContain('evalRemotionJSX(fontSafeCode)')
