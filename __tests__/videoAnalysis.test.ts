...
describe('video analysis provider contract', () => {
  it('uses 3.8 low thinking and preserves cache/thought usage without exposing thinking text', async () => {
    ...
    expect(body.contents[0].parts[0].fileData.mimeType).toBe('video/webm');
  ...
  it.each([429, 500, 503])('does not re-download or re-submit a video for HTTP %s', async status => {
    ...
    await expect(analyzeVideoWithProvider('https://example.com/a.mp4')).rejects.toThrow('Provider unavailable');
  ...
  it('does not turn configuration errors into inline retries', async () => {
    ...
    await expect(analyzeVideoWithProvider('https://example.com/a.mp4')).rejects.toThrow('Invalid thinking');
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it('retries a rejected file URI once as inline data using the same selected model', async () => {
    const fetchMock = setup();
    fetchMock.mockResolvedValueOnce(Response.json({ error: { message: 'Unsupported file URI' } }, { status: 400 }))
    ...
    expect(JSON.parse(fetchMock.mock.calls[2][1].body).contents[0].parts[0].inlineData.data).toBe('AQID');
  });

  it('uploads larger fallback videos as Files and deletes only that temporary file', async () => {
    const fetchMock = setup();
    fetchMock.mockResolvedValueOnce(Response.json({ error: { message: 'Unsupported file URI' } }, { status: 400 }))
      .mockResolvedValueOnce(new Response(new Uint8Array(14_000_001)))
      .mockResolvedValueOnce(Response.json(success));
    ...
    const part = JSON.parse(fetchMock.mock.calls[2][1].body).contents[0].parts[0];
  ...
  it('rejects a silent static response to an agentic request', async () => {
    ...
    await expect(analyzeVideoWithProvider('https://example.com/a.mp4')).rejects.toThrow('not confirmed');
  });
  ...
  it('does not accept or retry a truncated answer', async () => {
    ...
    await expect(analyzeVideoWithProvider('https://example.com/a.mp4')).rejects.toThrow('MAX_TOKENS');
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });
  ...
  it('does not disclose the key or signed source URLs in a tool error', async () => {
    ...
    await expect(analyzeVideoWithProvider('https://example.com/a.mp4')).rejects.toThrow('Invalid [redacted] [media URL]');
  });
});
