...
describe('Remotion export worker contract', () => {
  ...
  it('renders MP4s server-side and records workspace output', () => {
    ...
    expect(exporter).toContain("process.env.REMOTION_RENDERER === 'lambda'")
    ...
    expect(exporter).toContain('extractVideoPoster')
    expect(exporter).toContain('uploadPoster')
    ...
    expect(read('src/lib/remotion-lambda-renderer.ts')).toContain("REMOTION_LAMBDA_FRAMES_PER_LAMBDA, 20")
