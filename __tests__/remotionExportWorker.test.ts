...
describe('Remotion export worker contract', () => {
  ...
  it('renders MP4s server-side and records workspace output', () => {
    ...
    expect(exporter).toContain('drainRemotionExportQueue')
    ...
    expect(read('src/lib/remotion-export-capacity.ts')).toContain('DEFAULT_REMOTION_LAMBDA_FRAMES_PER_LAMBDA = 20')
    expect(read('src/lib/remotion-lambda-renderer.ts')).toContain('resolveFramesPerLambda(')
    expect(read('src/lib/remotion-export-capacity.ts')).toContain('MAX_REMOTION_LAMBDAS_PER_RENDER')
  ...
  it('exposes API and CLI entrypoints for composition export', () => {
    ...
    expect(agent).toContain('drainRemotionExportQueue')
    expect(agent).toContain('shouldRunRemotionExportInline')
    ...
    expect(videoSnapshotRoute).toContain('drainRemotionExportQueue')
    ...
    expect(remotionExportCron).toContain('REMOTION_EXPORT_CRON_LANES')
    expect(remotionExportCron).toContain('drainRemotionExportQueue')
    expect(read('vercel.json')).toContain('/api/cron/remotion-export')
    expect(capacityMigration).toContain("pg_advisory_xact_lock(hashtext('remotion-export-capacity-v1'))")
    expect(capacityMigration).toContain('FOR UPDATE OF j SKIP LOCKED')
    expect(capacityMigration).toContain("metadata->>'estimatedLambdaSlots'")
    expect(capacityMigration).toContain('GRANT EXECUTE')
