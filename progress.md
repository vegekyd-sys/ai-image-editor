...
## 性能调研：Tips/Preview 速度优化（2026-03-08）

### 背景
用户想了解从新建项目到看到第一个 tips preview 的完整耗时链路，以及可能的优化空间。

### Google Direct vs OpenRouter 生图速度对比
用 `gemini-3.1-flash-image-preview` 测试，结果出乎意料：

| Provider | Text-to-Image | Image Edit (454KB) |
|----------|--------------|-------------------|
| Google Direct | 17.1s | 35.8s |
| OpenRouter | 10.6s | 11.1s |

**结论：OpenRouter 比 Google 直连快 2-3 倍**，尤其 image edit 场景。线上已用 OpenRouter，无需切换。

### 完整链路 Profiling（线上，文生图场景）
通过 Chrome DevTools MCP 在 makaron.app 线上环境抓取 network timing：

```
T+0s      POST /api/agent 发出
T+8.7s    Agent 决定调用 generate_image
T+27.6s   Agent IMAGE 到达 (Gemini 18.9s, 图片 2225KB)
T+28s     4x POST /api/tips 并发（各 2.2MB base64）
T+47s     Tips SSE 全部返回（含 editPrompt）
T+47s     POST /api/preview 开始
T+60s     第一个 preview 返回
```

**时间分解**：Agent 思考 8.7s → Gemini 生图 18.9s → Tips 生成 ~19s → Preview 生图 ~13s

### 尝试的优化：等 Supabase URL 再发 Tips（已回退）

**假设**：Agent 生图后 tips 请求传 2.2MB base64 x4 = 8.8MB 上传太大，等 Supabase upload 完成后用 URL (~100B) 发请求应该更快。

**实际结果**：

| 指标 | 优化前 | 等 URL 后 |
|------|--------|----------|
| Tips payload | 2.2MB x4 | 340KB x4（压缩 fallback）|
| 第一个 preview | T+47s | T+59s |
| Preview 500 | 1 个 | 0 个 |

**等 URL 反而慢了 ~12s**，原因：
1. 上传 2.2MB 到 Supabase 需要 3-5s，这段时间 tips 被阻塞
2. Tips 生成耗时由 **Gemini 推理时间主导**（~15-20s），和上传 payload 大小关系不大
3. 带宽节省不等于延迟改善——发请求的 overhead 不是瓶颈，AI 推理才是

**关键经验**：
- 对于 AI API 调用，**延迟 = 推理时间 >> 网络传输时间**，减少 payload 不改善用户感知速度
- "等一个更优 input 再发请求" 的模式在 AI 场景下通常是负优化——**越早发越好**
- 现有方案（立即用 base64 发 tips，后台异步上传 Supabase）已经是最优策略
- 真正能改善的只有 Gemini 推理速度本身（换更快模型、减少 prompt 长度等）

### Vercel 在项目中的角色
- Serverless Functions 运行所有 /api/* 路由（tips、preview、agent 等）
- Function Region `hnd1`（东京），与 Supabase 同区域
- 长时间 API 调用（preview 120s、rotate 300s）需要 Pro 计划（Hobby 限 10s）
- 前端静态资源通过 Vercel Edge CDN 全球分发
- 速度瓶颈在 AI 推理，不在 Vercel/Supabase

---

## Editor 图片加载优化（2026-03-08）

### 问题发现（Chrome DevTools Profiling）
项目 "Childhood Play Time"（9 snapshots, 82 tips, 34 previews）：
- **所有 AI 生图存储为 PNG ~2.5MB**，虽然文件名是 `.jpg`
- 根因：`gemini.ts` 的 `part.inlineData.mimeType || 'image/png'` 默认值
- Editor 页加载 7 张全尺寸 PNG = **~18.4MB**
- CUI inline 图片、TipsBar 缩略图全部加载原图

### 三层优化

**Opt 1: AI 生图 PNG→JPEG**
- `gemini.ts` 新增 `ensureJpeg(dataUrl)` 用 Sharp quality 95 转换
- 应用到 Google SDK + OpenRouter 共 6 个图片输出点
- 新图 ~2.5MB PNG → ~0.7MB JPEG（-72%）

**Opt 2: Supabase Image Transformations 分层**
- `storage.ts` 新增 `getOptimizedUrl(url)` → `/render/image/?width=2000&quality=95`
- Canvas 主图：`getOptimizedUrl` PNG→WebP ~200KB（无可见画质损失，-84%）
- 远处 snapshot：`getThumbnailUrl(url, 800, 75)` ~50KB
- CUI inline：`getThumbnailUrl(url, 680, 75)` ~70KB（-96%）
- TipsBar：`getThumbnailUrl(url, 144, 60, 144)` ~4KB
- Before/After 对比：`getOptimizedUrl` 保持与 canvas 一致

**Opt 3: Draft preview 过渡动画**
- 点击 tip → 先显示 `getThumbnailUrl(url, 144, 60, 144, 'contain')` 低清缩略图
- Canvas 上方 shimmer 光条动画（1.5s 循环）
- 全图 preload 完成后无缝替换，shimmer 消失
- 解决了原来点击 tip 后 canvas 黑屏等待的问题

### Profiling 对比（"Godzilla Mall Invasion"，4 snapshots）

| 指标 | 优化前 | 优化后 | 改善 |
|------|--------|--------|------|
| 图片总传输 | 7.49MB | ~1.42MB | **-81%** |
| Canvas 主图 | 1.76MB PNG | 217KB WebP | -88% |
| CUI inline (3张) | 6.77MB PNG | 252KB WebP | -96% |
| TipsBar 缩略图 | 全尺寸原图 | 4KB WebP | -99% |
| `/object/` 请求数 | 4 | 1（仅原始上传） | `/render/` 全覆盖 |

### 关键踩坑
1. Supabase `/render/image/` **只在 width < 原图宽度时才转格式**。width=2048 对 2048px 图不生效，width=2000 可以
2. `width=144` 不带 height 时 Supabase 用 fill 模式只约束宽，高度保持原始 → 长条。加 `height+resize=contain` 修复
